...
namespace {
   bool copyLocal2Alien(const char* where, const char* loc, const char* rem) {
      TString local_md5 = gSystem->GetFromPipe(Form("md5sum %s | awk '{print $1}'", loc));
      TString remote_md5 = gSystem->GetFromPipe(Form("alien.py md5sum alien://%s | awk '{print $1}'", rem));
      if (local_md5 == remote_md5) {
          Info(where, "Remote file alien://%s already exists, skipping upload.", rem);
          return true;
      }
      auto command = Form("alien.py cp file://%s alien://%s", loc, rem);
      auto ret = gSystem->Exec(command);
      if (ret != 0) { 
          Warning(where, "Failed to copy %s to alien://%s", loc, rem);
          return false;
      }
      return true;
   }
...
//      CdWork();
	    ...
	    // Write standard error to null device
         ...
         if (ncount == gMaxEntries) {
	      ...
	      // cholm - Use AddFast (via interpreter)
               ...
               gSystem->Exec(Form("cp __tmp%d__%s %s",stage, file.Data(), file.Data()));
            }
            gSystem->Exec("rm -f __tmp*");   
      ...
      if (!TestBit(AliAnalysisGrid::kTest) && (!fileExists || fOverwriteMode)) {
        ...
        copyLocal2Alien("CreateDataset", file.Data(), Form("%s/%s",workdir.Data(), file.Data()));
         if (!FileExists(file)) {
...
//         CdWork();
	 ...
	 // cholm - Identical loop - should be put in common function for code simplification
         ...
         if (fNrunsPerMaster<2) {
            ...
            if (((nruns-1)%fNrunsPerMaster) == 0) {
               schunk = os->GetString();
               cbase = dynamic_cast<TAliceCollection*>(gGrid->OpenCollection(file.Data(), 1000000));
            } else {
               cadd = dynamic_cast<TAliceCollection*>(gGrid->OpenCollection(file.Data(), 1000000));
	       ...
	       // cholm - Use AddFast (via interpreter)
            ...
            copyLocal2Alien("CreateDataset", schunk.Data(), Form("%s/%s",workdir.Data(), schunk.Data()));                        
